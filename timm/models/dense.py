...
class Attention(nn.Module):
    def __init__(
        ...
        self.qkv = nn.Linear(self.dim, self.dim * 3, bias=qkv_bias, **dd)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(self.dim, self.dim, **dd)
        self.proj_drop = nn.Dropout(proj_drop)
        self.dense_token = nn.Parameter(
            torch.zeros(1, self.num_heads, dense_token, self.head_dim, **dd)
        )
        trunc_normal_(self.dense_token, std=0.02)
    ...
    def forward(
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        dense_token = F.scaled_dot_product_attention(
            self.dense_token.expand(B, -1, -1, -1),
            k,
            v,
            dropout_p=self.attn_drop.p if self.training else 0.0,
        )
        dense_token = dense_token.transpose(1, 2).reshape(B, -1, self.dim)
        dense_token = self.proj(dense_token)

        return x, dense_token
...
class Block(nn.Module):
    ...
    def forward(
        ...
        res = x
        x, dense_token = self.attn(self.norm1(x))
        x = res + self.drop_path1(x)
        x = x + self.drop_path2(self.mlp(self.norm2(x)))
        x = torch.cat((x, dense_token), dim=1)
        return x
    ...
    def reset_classifier(
        ...
        if global_pool is not None:
            ...
            if global_pool in ("map", "prr") and self.attn_pool is None:
                ...
                assert (
                    False
                ), "Cannot currently change attention pooling type in reset_classifier()."
            self.global_pool = global_pool
        self.head = (
...
@register_model
def dense_tiny_patch16_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
...
@register_model
def dense_small_patch16_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
...
@register_model
def dense_base_patch16_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
